...
public class StoreTest {
    ...
    public void testGetTask_TwoOrders_SinglePicker() {
        ...
        Order order2 = new Order("order-2", "5", "PT40M", "10:00");
        orders.add(order1);
        orders.add(order2);
    ...
    public void testStore() {
        ...
        System.setOut(new PrintStream(outContent));
        try {
            Store.store("..\\ocado_task1\\testData\\store.json", "..\\ocado_task1\\testData\\orders.json");
        } catch (IOException | ParseException e) {
            System.out.println(e.getMessage());
        }
        String expectedOutput = "P1 order-1 09:00\r\n";
        Assert.assertEquals(expectedOutput, outContent.toString());
    }
}
